...
static inline void sort4(u32* arr) {
...
static inline u32 subindex2(u32 j) {
    return (j*(j+1))/2;
}

static inline u32 subindex3(u32 k) {
    return (k*(k+1)*(k+2))/6;
}

static inline u32 subindex4(u32 l) {
...
static inline u32 index4(u32 i, u32 j, u32 k, u32 l) {
    u32 arr[4] = {i, j, k, l};
    sort4(arr);

    return arr[0] + subindex2(arr[1]) + subindex3(arr[2]) + subindex4(arr[3]);
}
...
static inline u64 size4(u64 N) {
	return index4(N-1, N-1, N-1, N-1) + 1;
}
