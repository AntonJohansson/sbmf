f64 hermite_integral_4(u32 i, u32 j, u32 k, u32 l) {
	...
	f128 fi = factorial_128(i);
	f128 fj = factorial_128(j);
	f128 fk = factorial_128(k);
	f128 fl = factorial_128(l);
	...
	for (u32 m = 0; m <= m_max; ++m) {
		...
		sum +=
			((pow(2,m)/f1)*sqrt(fi/pow(2,i)))
			*((df1/f2)*sqrt(fj/pow(2,j)))
			*((df2/f3)*sqrt(fk/pow(2,k)))
			*df3*sqrt(fl/pow(2,l));
	}
