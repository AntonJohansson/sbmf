...
static f64 _lambda0;
static f64 _lambda;
static f64* _orbital_1_coeffs;
static f64* _orbital_2_coeffs;
...
static void op_1(const u32 len, f64 out[static len],
	...
	for (u32 i = 0; i < len; ++i) {
		f64 abs1 = fabs(in_u[0*len + i]);
		f64 abs2 = fabs(in_u[1*len + i]);
...
static void guess_1(f64* out, u32 len) {
	for (u32 i = 0; i < len; ++i) {
...
static void guess_2(f64* out, u32 len) {
	for (u32 i = 0; i < len; ++i) {
		f64 c1 = sqrt((f64)_n1/(f64)_particle_count);
		f64 c2 = sqrt((f64)_n2/(f64)_particle_count);
...
static void ensure_structure_of_func(f64* a, f64* b, u32 len) {
	f64 sum = 0.0;
	for (u32 i = 0; i < len; ++i) {
		sum += conj(a[i])*b[i];
	}
	 ...
	 * => n1 = (N-(n2-n1))/2
	 */
	//f64 n1 = ((f64)_particle_count - n2_minus_n1)/2.0;
	//f64 n2 = (f64)_particle_count - n1;
