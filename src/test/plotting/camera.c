...
void camera_set_mode(camera* cam, i32 mode) {
	switch (mode) {
		case CAM_PAN:
			cam->mode = mode;
			camera_update_perspective_projection(cam);
			camera_update_pan(cam);
			break;
		case CAM_ARCBALL:
			cam->mode = mode;
			//camera_update_perspective_projection(cam);
			//m4identity(&cam->projection);
			camera_update_perspective_projection(cam);
			camera_update_arcball(cam);
			break;
		default:
			assert(0);
	};
